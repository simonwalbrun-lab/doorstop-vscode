...
- Q: What should happen if the user turns on Ghost Preview while Hierarchical Layout mode is also active, since Hierarchical Layout already takes over node positioning and disables physics entirely? → A: (SUPERSEDED by spec 015) Originally: mutually exclusive — turning either mode on disabled the other's toggle. Spec 015 makes body items permanently static and turns Hierarchical into a one-shot arrange command, so the two no longer conflict and neither disables the other.
...
- **FR-014**: ~~Ghost Preview mode and the existing Hierarchical Layout mode MUST be mutually exclusive~~ — **REMOVED, superseded by spec 015 (FR-024)**. Body items are now permanently static and the hierarchical arrangement is a one-shot command, so no diagram toolbar control is disabled as a side effect of another control's state.
